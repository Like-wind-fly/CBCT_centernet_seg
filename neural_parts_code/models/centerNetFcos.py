...
class CenterNetFcosBuilder(object):
    ...
    def head(self):
        if self._head is None:
            ...
            self._head = [heatmap,classify,fcos_town,reg,dwh,regress_town]
...
class CenterNet(nn.Module):
    def __init__(self, encode,decode,town,head,config):
        ...
        self.decoder = decode
        self.town = town
        heatmap,classify,fcos_town,reg,dwh,regress_town = head
        self.heatmap = heatmap
        self.reg = reg
        self.dwh = dwh
        self.classify = classify
        self.regress_town = regress_town
        self.fcos_town = fcos_town
        self.config = config
    ...
    def forward(self, input):
        ...
        feature_map1 = self.regress_town(feature_map)
        feature_map2 = self.fcos_town(feature_map)

        reg = self.reg(feature_map1) / 2**used_layer
        dwh = self.dwh(feature_map1)

        hm = self.heatmap(feature_map2)
        classify = self.classify(feature_map2)

        predictions = {}

        if self.config["head"].get("classifyEqheatmap", False):
            predictions["heatmap"] = classify
        else:
