...
class CenterNetMutiBuilder(object):
    ...
    def head(self):
        if self._head is None:
            ...
            classify = nn.Sequential(
                nn.Conv3d(num_channels_in, num_channels_in, 3, 1, padding=1),
                nn.LeakyReLU(0.2),
                nn.Conv3d(num_channels_in, 8 , 1, 1)
                # nn.Softmax()
            )

            self._head = [heatmap,reg,dwh,classify]
...
class CenterNetMuti(nn.Module):
    def __init__(self, encode,decode,town,head,config):
        super(CenterNetMuti, self).__init__()
        self.encoder = encode
        self.decoder = decode
        self.town = town
        heatmap,reg,dwh,classify = head
