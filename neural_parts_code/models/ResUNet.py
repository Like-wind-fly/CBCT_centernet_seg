...
class ResUNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        outputs = self.decoder_stage1(short_range4) + short_range4
        # /16 c256
        ...
        outputs = self.decoder_stage2(torch.cat([short_range6, long_range4], dim=1)) + short_range6
        outputs = F.dropout(outputs, self.dorp_rate, self.training)
        short_range7 = self.up_conv3(outputs)
        # /4 c64

        outputs = self.decoder_stage3(torch.cat([short_range7, long_range3], dim=1)) + short_range7
        outputs = F.dropout(outputs, self.dorp_rate, self.training)

        short_range8 = self.up_conv4(outputs)
        # /2 c32

        outputs = self.decoder_stage4(torch.cat([short_range8, long_range2], dim=1)) + short_range8

        short_range9 = self.up_conv5(outputs)
        output4 = self.map4(short_range9)

        return output4

if __name__ == "__main__":
    x = torch.rand(8,2,64,64,64)
    unet = ResUNet(in_channel=2, out_channel=1)
    out = unet.forward(x)
    print(out.shape)