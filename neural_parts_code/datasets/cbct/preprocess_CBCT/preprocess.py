...
class CBCT_preprocess:
    ...
    def fix_data(self):
        ...
        file_list = os.listdir(join(self.raw_root_path, 'cbct_label_niigz'))
        ...
        for seg_file, i in zip(file_list, range(Numbers)):
            ...
            seg_path = os.path.join(self.raw_root_path, 'cbct_label_niigz', seg_file)
            ct_path = os.path.join(self.raw_root_path, 'cbct_volume_data', seg_file.replace('seg_',''))
            filename = seg_file[:-len(".nii.gz")]
            save_path = os.path.join(self.fixed_path, "{}.pickle".format(filename.replace('seg_','')))
            #save_path = os.path.join(self.fixed_path,"{}.pickle".format(filename))
    ...
    def resize(self, ct_path, seg_path):
        ...
        ct_array = ct_array - self.args.median
        ...
        # 截取保留区域
        ct_array = ct_array[xyz_range[0][0]:xyz_range[0][1], xyz_range[1][0]:xyz_range[1][1],xyz_range[2][0]:xyz_range[2][1]]
        seg_array = seg_array[xyz_range[0][0]:xyz_range[0][1], xyz_range[1][0]:xyz_range[1][1],xyz_range[2][0]:xyz_range[2][1]]
        print("Preprocessed shape:", ct_array.shape, seg_array.shape)
        max_slice = max(ct_array.shape[1],ct_array.shape[2],ct_array.shape[0])
        zoom_size = 256/max_slice
        # return zoom_size
        ct_array = ndimage.zoom(ct_array, (zoom_size, zoom_size, zoom_size), order=3)
        seg_array = ndimage.zoom(seg_array, (zoom_size, zoom_size, zoom_size), order=0)

        min_size = [256,256,256]
        pad = np.array(min_size ) - np.array(seg_array.shape)
        pad[pad<=0] = 0
        pading = []
        for i in range(3):
        ...
        ct_array = np.pad(ct_array, pading , 'constant', constant_values=0)
        seg_array = np.pad(seg_array, pading, 'constant', constant_values=0)
...
if __name__ == '__main__':
    raw_dataset_path = '/mnt/fourT/our_cbctdata'
    fixed_dataset_path = '/mnt/fourT/our_cbctdata/pickle'
